...
export default function ReviewAdminPage() {
  ...
  const handleToggleFeature = async (id: string, currentFeatured: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        setReviews(prev => prev.map(r => r.id === id ? { ...r, isFeatured: !currentFeatured } : r));
      }
    } catch (err) { console.error(err); }
    setActionLoading(null);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-2">Approve, reject, feature, or inject customer feedback.</p>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
        {loading ? (
          <div className="p-12 text-center text-gray-500 font-bold flex flex-col items-center gap-3">
            <Loader2 className="animate-spin" size={32} /> Loading Reviews...
          </div>
        ) : (
                ...
                <tr key={review.id} className={`hover:bg-gray-50/50 transition-colors ${actionLoading === review.id ? 'opacity-50' : ''}`}>
                  ...
                  <td className="p-6 align-top text-center">
                    <div className="flex flex-col gap-2 items-center">
                      ...
                      {review.isFeatured && (
                         <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded border border-yellow-200">
                  ...
                  <td className="p-6 align-top text-right">
                    <div className="flex items-center justify-end gap-3">
                      {/* Feature/Crown Toggle */}
                      <button 
                        onClick={() => handleToggleFeature(review.id, review.isFeatured)} 
                        className={`p-2 rounded-lg transition-colors ${review.isFeatured ? 'bg-yellow-100 text-yellow-600' : 'text-gray-400 hover:text-yellow-600 hover:bg-yellow-50'}`} 
                      ...
                      <button onClick={() => handleUpdateStatus(review.id, 'approved')} className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Approve">
                        <CheckCircle size={18} />
                      </button>
                      <button onClick={() => handleUpdateStatus(review.id, 'rejected')} className="p-2 text-gray-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Reject">
                        <XCircle size={18} />
                      </button>
                      <button onClick={() => handleDelete(review.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
